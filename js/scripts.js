...
jQuery(document).ready(function($) {
	...
	*******************************************/
    ...
    var $fbLike = $('.fb-like-button');

    /// Age Gate Variables----------------------------------------------
    var $ageGate = $('.age-gate');
    var $ageGateContent = $('.inner-wrap.restricted');
    var $ageGateRequirement = 13;
    var $ageGateCookieFail = "pwnd-age-gate-fail";
    var $ageGateCookiePass = "pwnd-age-gate-pass";
    var $ageGateCookieExpireDays = 20 * 365;
    var $ageGateDenied = $('.age-gate .restricted');
    var $ageGateInput = $('.age-gate .input-lg');
    var $ageGateMonth = $('#month');
    var $ageGateDay = $('#day');
    var $ageGateYear = $('#year');
    var $ageGateForm = $("#age-gate-form");
    /// ----------------------------------------------------------------

    ///////////////////////////////////////////////////////////////////////
    /////////////////////  Age Gate  /////////////////////////
    //////////////////////////////////////////////////////////////////////
    var ageGateFail = getCookie($ageGateCookieFail);
    if (ageGateFail != "") {
        $ageGateForm.hide();
        $ageGateDenied.show();
        $ageGateContent.remove();
        $ageGate.addClass("visible");
    } else {
        var ageGatePass = getCookie($ageGateCookiePass);
        if (ageGatePass != "") {
            $ageGateContent.addClass("visible");
            $fbLike.append('<div class="fb-like" data-href="//www.facebook.com/pwndgame" data-layout="button_count" data-action="like" data-show-faces="false" data-share="false"></div>');
            $ageGate.remove();
        } else {
            $ageGateContent.remove();
            $ageGate.addClass("visible");
    ...
    $ageGateInput.keyup(function (e) {
        ...
        if (e.keyCode == 13 || maxchar) {
            var current = $ageGateInput.index(this),
                next = $ageGateInput.eq(current + 1).length ? $ageGateInput.eq(current + 1) : $ageGateInput.eq(0);
            if (next.val() == '') {
                next.focus();
            }
        }
    ...
    $ageGateForm.on('submit', function() {
        $ageGateForm.addClass('animated fadeOut');
        var birthday = +new Date($ageGateYear.val() + "-" + $ageGateMonth.val() + "-" + $ageGateDay.val());
        var age =  ~~((Date.now() - birthday) / (31557600000));
        if (age < $ageGateRequirement) {
            setCookie($ageGateCookieFail, 'fail', $ageGateCookieExpireDays);
            setTimeout(function(){
                $ageGateForm.hide();
                $ageGateDenied.show();
            }, 1000);
        } else {
            setCookie($ageGateCookiePass, 'pass', $ageGateCookieExpireDays);
            location.reload();
        }
        return false;
    });
	...
	*******************************************/
    ...
    /*Popovers
     *******************************************/
    $popover.popover({delay: { "hide": 1500 }});
